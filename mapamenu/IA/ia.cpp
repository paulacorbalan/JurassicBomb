...
void IA::movimientoDinos(vector<Dinosaurio*> dinosaurios, int _cont,std::vector<sf::Sprite*> &todoSprite, Map &mapas,float times){
    ...
    width_map = mapas.getwidth();
        ...
        for(int i=0; i <  dinosaurios.size(); i++){
            ...
            if(dir == 0){
                dinosaurios[i]->marriba(todoSprite, numlayers, height_map, width_map, tilemapSprite, mapas,times);
            }
            if(dir == 1){
                dinosaurios[i]->mabajo(todoSprite, numlayers, height_map, width_map, tilemapSprite, mapas,times);
            }
            if(dir == 2){
                dinosaurios[i]->mderecha(todoSprite, numlayers, height_map, width_map, tilemapSprite, mapas,times);
            }
            if(dir == 3){
                dinosaurios[i]->mizquierda(todoSprite, numlayers, height_map, width_map, tilemapSprite, mapas,times);
            }
            else{
